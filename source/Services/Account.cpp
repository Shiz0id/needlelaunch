...
Result Account::SetCustomProfileImage(std::string filename) {
	...
	size_t* buffer;
        rc = accountProfileGet(&acc, &ud, &pb);
        rc = accountProfileLoadImage(&acc, &ud, buffer, fsize);
...
Result Account::SetProfilePicture(u128 userId, u8 *jpg, size_t jpgSize) {
	...
	AccountProfile prof;
	AccountProfileBase pb;
	AccountUserData* accoData;
	AccountUserData  accData;
	rc = accountProfileGet(&prof, accoData, &pb);
	rc = accountProfileLoadImage(&prof, NULL, &jpgSize, &jpgSize);
	#endif
	return rc;
}
