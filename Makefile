...
CFLAGS	:=	-g -fpermissive -ffunction-sections $(ARCH) $(DEFINES) $(INCLUDE) -D__SWITCH__ -DTITLE='"$(TITLE)"'

CXXFLAGS	:= $(CFLAGS) -fno-rtti -O2 -s -fno-exceptions -std=gnu++17 -lstdc++fs

ASFLAGS	:=	-g $(ARCH)
LDFLAGS	=	-lnx -specs=$(DEVKITPRO)/libnx/switch.specs -g $(ARCH) -Wl,-Map,$(notdir $*.map) -lwebp

LIBS	:= -lfreetype -lSDL2_mixer -lwebp -lmodplug -lmpg123 -lvorbisidec -logg -lSDL2_ttf -lSDL2_gfx -lSDL2_image -lpng -ljpeg `sdl2-config --libs` `freetype-config --libs` -lcurl -lmbedtls -lmbedcrypto -lmbedx509 -lz -lnx -lstdc++fs
